...
Everything above is self-contained --- no example so far needed anything beyond `contexture` itself. In practice, `contexture` is meant as a shared foundation that several packages build on at once. `@preview/palimpsest` (manuscript revision letters) and `@preview/checkitoff` (reporting-guideline checklists) are two such packages, published independently of each other and of `contexture`, neither aware the other exists. Combining them needs nothing beyond listing both of their satellites under the same `documents:` --- the scenario the whole design exists for:

#code-of("manual-snippets/bundle-combo-palimpsest-checkitoff.typ")
...
- Reporting-guideline checklists (CONSORT, PRISMA, SPIRIT, STARD, STROBE) that cite the real pages: `@preview/checkitoff`'s own manual.
